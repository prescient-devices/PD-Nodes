...
class FreePort {
  ...
  async get(minPort) {
    ...
    if (!ports.find((item) => this.ports.includes(parseInt(item)))) {
      const port = parseInt(ports[0])
      this.ports.push(port)
      return port
    } else {
      console.error("Cannot find a free port")
    }
  }
  release(port) {
    const idx = this.ports.findIndex((item) => item === port)
    this.ports.splice(idx, 1)
  }
}

class NodeRED {
  constructor(version, config, env, readOnly, port) {
    this.version = version
    this.config = config
    this.env = env
    this.readOnly = readOnly
    this.port = port
    this.rootDir = path.resolve(__dirname, `node-red-distro-${version}`)
    this.testDir = path.resolve(__dirname, `.node-red-${version}-${port}`)
  ...
  start() {
    const that = this
    return new Promise(function (resolve) {
      const nodeRedBin = path.resolve(that.rootDir, "node_modules", ".bin", "node-red")
      try {
      ...
      if (!fs.existsSync(that.rootDir)) {
        spawnSync("npm", [
          "install",
          "--omit=dev",
          "--prefix",
          path.resolve(that.rootDir),
          `node-red@${that.version}`,
        ])
      }
      spawnSync("npm", [
        "install",
  ...
  stop() {
    ...
    PORTS.release(this.port)
  }
}

const PORTS = new FreePort()
...
async function runTest({
  ...
  try {
    const port = await PORTS.get()
    if (isNaN(port)) {
      return
    }
    this.test.nodeRedObj = new NodeRED(nodeRedVersion, config, env, readOnly, port)
...
function testSuite(browserDesc, nodeRedVersion) {
  const browserCls = browsers[browserDesc]
  describe(`node-red-contrib-promptinput (browser: ${browserDesc}, Node-RED version: ${nodeRedVersion})`, function () {
    afterEach(function () {
